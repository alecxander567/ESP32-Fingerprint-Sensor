    ...
    if (currentMode == "enroll") {
        ...
        while (p != FINGERPRINT_OK) {
            ...
            if (millis() - startTime > 15000) {
                ...
                httpTimeout.begin(String(serverUrl) + "recognition-result?finger_id=0&matched=false");
                httpTimeout.GET();
                httpTimeout.end();
                currentMode = "idle";  
                return;
        ...
        if (p == FINGERPRINT_OK) {
            ...
            ledSuccess();
            beepSuccess();
            resultUrl = String(serverUrl) + "recognition-result?finger_id="
                        + String(finger.fingerID) + "&matched=true";
        } else {
